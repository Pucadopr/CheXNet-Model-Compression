...
def finetune_pretrained_model(model, compress_type, batch_size, log_interval, percent=0.2):
    ...
    # d_nih = NIHDataset(image_path, x_ray_path, bbox_path, transform=transform, normalize=True)
    ...
    print(f"Using a total of {no_test_data} for model validation")
    ...
    pretrained_model_accuracy(d_nih, model, "cpu")
    # print(f"Initial accuracy of model is {accuracy} %")
    
    if compress_type=='prune':
        modeltwo = pr.random_structured_pruning(model)

        print_size_of_model(modeltwo)
    ...
    if compress_type=='quantize':
        model.eval()
        quantmodel = quant.static_quantize(model)
        
        print_size_of_model(quantmodel)

        pretrained_model_accuracy(d_nih, quantmodel, "cpu")